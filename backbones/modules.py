...
class StructurePositionLayer(Operation):
    """
    This custom version is for only fixed number of 4 qubits
    """
    num_params = 1
    ...
    def expand(self):
        ...
        with qml.tape.QuantumTape() as tape:
            for layer in range(repeat):
                ...
                qml.CNOT(wires=self.wires.subset([2, 3]))
                qml.CNOT(wires=self.wires.subset([1, 2]))
                qml.CNOT(wires=self.wires.subset([2, 4]))
        return tape
...
class RQCNN(nn.Module):
    def __init__(self, kernel_size, depth, device, circuit_layers=1, method="random"):
        ...
        elif method =='structure_position':
            self.qlayer = qml.qnn.TorchLayer(structure_position_circuit, params)
...
class SPQCNN(nn.Module):
    def __init__(self, kernel_size, depth, device, circuit_layers=1, method="random"):
        ...
        @qml.qnode(dev)
        def structure_position_circuit(inputs, weights):
            for j in range(inputs.shape[0]):
                qml.RY(np.pi * inputs[j], wires=j)

            StructurePositionLayer(weights, wires=list(range(self.n_qubits)))
    ...
    def forward(self, x):
        ...
        for b in range(x.shape[0]):
            for i, row in enumerate(range(x.shape[3] - self.kernel_size + 1)):
                for col in range(x.shape[2] - self.kernel_size + 1):
                    for channel in range(x.shape[1]):
                        q_out[b, row, col] += self.qlayer(
                            self.flatten(x[b, channel, row:row + self.kernel_size, col:col + self.kernel_size], i))
        return torch.reshape(q_out, (
        x.shape[0], self.depth, x.shape[3] - self.kernel_size + 1, x.shape[3] - self.kernel_size + 1))
