...
idx = (X_train.targets == 0) | (X_train.targets == 6) | (X_train.targets == 9)
...
if ap.method == 'classical':
    net = CNN(kernel_size=2, depth=4, num_classes=3)
else:
    net = Hybrid_RQCNN(kernel_size=2, depth=4, circuit_layers=1, device=device, method=ap.method, num_classes=3).to(device)
loss_fun = nn.CrossEntropyLoss()
optimizer = torch.optim.Adam(net.parameters(), lr=1.e-3)


if not os.path.exists(ap.save_log):
    os.makedirs(ap.save_log)
...
for epoch in range(ap.num_epoches):
    ...
    if (epoch+1) % (ap.n_test) == 0:
        print('[INFO] Epoch [%d/%d]' % (epoch+1, ap.num_epoches))
    ...
    torch.save(state, os.path.join(ap.save_pretrained,'Hybrid_QCNN_epoch_{}.pth'.format(epoch+1)))
